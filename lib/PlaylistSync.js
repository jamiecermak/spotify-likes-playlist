...
function batchOperationStack(partitionedOperationStack) {
    ...
    for (let i = 0; i < partitionedOperationStack.length; i++) {
        ...
        if (action === 'add') {
            ...
            // If there is only one operation, add it and continue
            if (operationStack.length === 1) {
                batchOperations.push({
                    action: 'batch-add',
                    index: operationStack[0].index,
                    tracks: operationStack.map((op) => op.track),
                })
...
async function commitBatchAddOperation(spotifyApi, playlistId, operation) {
    ...
    Logger.info({
        ...
        message: `Performing BATCH-ADD Operation`,
        playlistId,
        operation,
    })
...
async function commitBatchRemoveOperation(
    ...
    Logger.info({
        service: 'playlist-sync',
        message: `Performing BATCH-REMOVE Operation`,
        playlistId,
        snapshotId,
        operation,
    })
...
async function commitMoveOperation(
    ...
    Logger.info({
        service: 'playlist-sync',
        message: `Performing MOVE Operation`,
        playlistId,
        snapshotId,
        operation,
    })

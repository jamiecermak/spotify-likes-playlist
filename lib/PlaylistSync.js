...
async function getSpotifyApiAndPlaylist(userId) {
    ...
    const playlist = await spotifyApi.getUserPlaylist(
        userId,
...
async function playlistSync(userId) {
    ...
    const likedTrackIds = likedTracks.map((track) => track.uri)
    const playlistTrackIds = playlistTracks.map((track) => track.uri)
